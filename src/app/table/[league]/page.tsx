...
export default function LeagueTable() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (response.status === 429) {
        ...
        if (result.stale) {
          setError('Warning: Using stale data from cache. Fresh data could not be loaded.');
        } else {
          setError(null);
        }
      }
  ...
  if (!leagueData) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-2 text-gray-600">The league you're looking for doesn't exist.</p>
  ...
  if (isRateLimited) {
    return (
      <div>
        <div className="mb-8">
          <Link href="/" className="inline-flex items-center text-sm font-medium text-emerald-600 hover:text-emerald-800">
          ...
          <div className={`${leagueData.bgColor} rounded-xl shadow-md overflow-hidden mt-4`}>
            <div className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-3xl font-bold text-white">{leagueData.name}</h1>
                  <p className="mt-1 text-white text-opacity-90">{leagueData.country}'s premier football competition</p>
                </div>
                <img src={leagueData.logo} alt={`${leagueData.name} logo`} className="h-16 w-auto object-contain" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden p-6 text-center">
          <div className="text-amber-600 text-xl mb-4">API rate limit exceeded</div>
          <p className="text-gray-600 mb-4">
            We've reached our limit for data requests. The table will automatically reload in:
          </p>
          <div className="text-2xl font-bold mb-6">{retryCountdown} seconds</div>
          <button 
            onClick={fetchData}
      ...
      <div>
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
          <div className="px-4 py-5 sm:px-6 border-b border-gray-200 flex justify-between items-center">
            <div>
              <h3 className="text-lg font-medium leading-6 text-gray-900">Current Standings</h3>
          ...
          <div className="flex items-center">
            {isCached && (
              <span className="text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded mr-2">
                Cached {cachedTime ? `at ${new Date(cachedTime).toLocaleTimeString()}` : ''}
              </span>
            )}
            <button 
              onClick={fetchData} 
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">W</th>
                <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">D</th>
                <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">L</th>
                <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">GF</th>
                <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">GA</th>
                <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">GD</th>
                <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Pts</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {tableData.map((team, index) => {
                const qualificationStatus = getQualificationStatus(team.position);
                ...
                return (
                  <tr key={index} className={`${positionStyle}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">{team.gf}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">{team.ga}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">{team.gd}</td>
        ...
        <div className="p-6">
          <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
            {leagueData.positions.championsLeague && (
              <div>
                <dt className="text-sm font-medium text-gray-500">Champions League</dt>
                <dd className="mt-1 text-sm text-gray-900">
                  Positions {leagueData.positions.championsLeague.join(', ')} qualify for the UEFA Champions League group stage.
                </dd>
              </div>
            )}
            ...
            {leagueData.positions.europaLeague && (
              <div>
                <dt className="text-sm font-medium text-gray-500">Europa League</dt>
                <dd className="mt-1 text-sm text-gray-900">
                  Positions {leagueData.positions.europaLeague.join(', ')} qualify for the UEFA Europa League group stage.
                </dd>
              </div>
            )}
            ...
            {leagueData.positions.conferenceLeague && (
              <div>
                <dt className="text-sm font-medium text-gray-500">Conference League</dt>
                <dd className="mt-1 text-sm text-gray-900">
                  Position {leagueData.positions.conferenceLeague.join(', ')} qualifies for the UEFA Europa Conference League.
                </dd>
              </div>
            )}
            ...
            {leagueData.positions.playoff && (
              <div>
                <dt className="text-sm font-medium text-gray-500">Relegation Playoff</dt>
                <dd className="mt-1 text-sm text-gray-900">
                  Position {leagueData.positions.playoff.join(', ')} enters a relegation playoff.
                </dd>
              </div>
            )}
            ...
            {leagueData.positions.relegation && (
              <div>
                <dt className="text-sm font-medium text-gray-500">Relegation</dt>
                <dd className="mt-1 text-sm text-gray-900">
                  Positions {leagueData.positions.relegation.join(', ')} are relegated.
                </dd>
              </div>
            )}
          </dl>
